  ...
  return (
    <div className="mx-auto mt-12 grid max-w-5xl gap-6 px-4 md:grid-cols-3">
      {skillGroups.map((group) => (
        <div key={group.title} className="border-2 border-black bg-white p-5 dark:border-white dark:bg-black">
          ...
          <ul className="flex flex-wrap gap-2">
            {group.items.map((item) => (
              <li
                key={item}
                className="text-text-primary dark:text-text-inverse border border-black bg-background-primary px-2 py-1 text-xs font-bold dark:border-white dark:bg-neutral-900"
              >
                {item}
              </li>
            ))}

...
const SectionHeader: React.FC<SectionHeaderProps> = ({
  ...
  return (
    <header className={`mb-12 lg:mb-16 ${alignClasses} ${className}`.trim()}>
      <p className="mb-3 text-sm font-bold uppercase tracking-[0.24em] text-primary-950 dark:text-primary-300">
        {eyebrow}
      </p>
      <h2 className="text-4xl font-black uppercase tracking-tight text-text-primary sm:text-5xl dark:text-text-inverse">
        {title}
      </h2>
      {description !== undefined ? (
        <p
          className={`mt-4 text-lg font-medium leading-8 text-text-secondary dark:text-neutral-400 ${descriptionAlignClasses}`}
        >
          {description}
        </p>
      ) : null}
      ...
      <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
        ...
        <ul className="grid content-start gap-4">
          {aboutContent.principles.map((principle) => (
            <li
              key={principle}
              className="border-2 border-black bg-white px-5 py-4 text-sm font-bold text-text-primary shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:border-white dark:bg-black dark:text-text-inverse dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]"
            >
              {principle}
            </li>
          ))}
...
return (
  ...
  >
    ...
    <div className="px-4 pb-8 text-center">{/* ... существующий h2 block ... */}</div>
...
<SectionHeader
  ...
  eyebrow={
    <InteractiveElement
      ...
      className="inline-block border-2 border-l-4 border-black border-l-primary-500 bg-background-primary px-3 py-1 text-sm font-bold uppercase tracking-[0.24em] text-text-primary dark:border-white dark:border-l-primary-400 dark:bg-background-tertiary dark:text-text-inverse"
    >
...
const FooterWidget: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="bg-linear-to-r absolute left-0 top-0 h-1 w-full from-transparent via-black/20 to-transparent dark:via-white/20" />
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background-primary">
      <Card variant="bgNone" padding="lg" className="mx-auto max-w-md text-center">
        ...
        <h2 className="mb-4 text-2xl font-bold text-text-primary">Произошла ошибка</h2>
        ...
        <div className="space-y-4">
          ...
          {process.env.NODE_ENV === "development" && (
            <details className="mt-4">
              <summary className="cursor-pointer text-sm text-text-muted hover:opacity-80">
                Подробности ошибки (dev)
              </summary>
              <pre className="mt-2 overflow-auto rounded-none border-2 border-black bg-background-secondary p-4 text-left text-xs text-text-tertiary dark:border-white">
                {error.message}
                {"\n"}
                {error.stack}
              </pre>

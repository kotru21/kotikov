...
describe("PuzzleHome", () => {
  ...
  it("frames tickers with overlaying strokes and K marks between phrases", () => {
    ...
    expect(desktopClasses).not.toContain("h-dvh");
    expect(desktopClasses).not.toContain("min-h-dvh");
    const header = document.querySelector("header#header");
    const headerClasses = header?.className.split(/\s+/) ?? [];
    expect(headerClasses).toContain("h-svh");
    expect(headerClasses).toContain("max-h-svh");
    expect(headerClasses).toContain("overflow-hidden");
    expect(headerClasses).toContain("[overflow-anchor:none]");
    expect(headerClasses).not.toContain("h-dvh");
  ...
  it("stacks mobile rows as Обо мне, Проекты, Опыт, Контакты", () => {
    ...
    expect(mobileShell).toHaveClass("flex", "h-full", "overflow-hidden");
    expect(mobileShell?.className.split(/\s+/)).not.toContain("h-dvh");

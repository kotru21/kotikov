...
import {
  ...
  getSlideClass,
  parsePeriodStart,
} from "@/widgets/timeline/ui/timelineUtils";
...
describe("timelineUtils", () => {
  ...
  it("getSlideClass respects reduced motion", () => {
    expect(getSlideClass(1, true)).toBe("");
    expect(getSlideClass(-1, true)).toBe("");
    expect(getSlideClass(1, false)).toBe("animate-timeline-slide-in-right");
    expect(getSlideClass(-1, false)).toBe("animate-timeline-slide-in-left");
  });
});
